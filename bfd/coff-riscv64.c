/* BFD back-end for RISCV-64 COFF files.
...
#define BADMAG(x) RISCV64BADMAG(x)
...
static bfd_boolean
coff_pe_riscv64_relocate_section (bfd *output_bfd,
...
#define coff_relocate_section coff_pe_riscv64_relocate_section

/* Convert an rtype to howto for the COFF backend linker.  */

static reloc_howto_type *
coff_riscv64_rtype_to_howto (bfd *abfd ATTRIBUTE_UNUSED,
			   asection *sec,
...
#define coff_bfd_reloc_type_lookup coff_riscv64_reloc_type_lookup
#define coff_bfd_reloc_name_lookup coff_riscv64_reloc_name_lookup

static reloc_howto_type *
coff_riscv64_reloc_type_lookup (bfd *abfd ATTRIBUTE_UNUSED, bfd_reloc_code_real_type code)
{
...
static reloc_howto_type *
coff_riscv64_reloc_name_lookup (bfd *abfd ATTRIBUTE_UNUSED,
			      const char *r_name)
{
  unsigned int i;
...
#define coff_rtype_to_howto coff_riscv64_rtype_to_howto

#ifdef TARGET_UNDERSCORE

/* If riscv64 gcc uses underscores for symbol names, then it does not use
   a leading dot for local labels, so if TARGET_UNDERSCORE is defined we treat
   all symbols starting with L as local.  */

static bfd_boolean
coff_riscv64_is_local_label_name (bfd *abfd, const char *name)
{
  if (name[0] == 'L')
    return TRUE;
...
#define coff_bfd_is_local_label_name coff_riscv64_is_local_label_name
...
#ifdef PE
#define riscv64coff_object_p pe_bfd_object_p
#else
#define riscv64coff_object_p coff_object_p
#endif
